class CitiesController < ApplicationController
  ...
  def index
    ...
    @cities = City.from("reports.cities")
    @cities = @cities.where("name ILIKE ?", "%#{params[:s][:name]}%") if params[:s]
    @cities = @cities.paginate(:page => params[:page] || 1, :per_page => params[:per_page] || 10)
  end
